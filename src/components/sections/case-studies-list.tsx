...
function excerpt(text: string | undefined, max = 160) {
  if (!text) return null;
  if (text.length <= max) return text;
  return `${text.slice(0, max).trim()}…`;
}

export function CaseStudiesList({ locale }: { locale: Locale }) {
  const { caseStudies, caseStudiesPage } = getLegacyContent(locale);
          ...
          <Reveal>
            ...
            >
              <div className="relative aspect-[4/3] bg-canvas md:aspect-auto md:min-h-[22rem]">
                {getCaseStudyImage(featured.slug) ? (
                  <SiteImage
                    src={getCaseStudyImage(featured.slug)!}
                    alt={featured.title}
                    fill
              ...
              <div className="flex flex-col justify-center p-8 md:p-10 lg:p-12">
                <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-accent">
                  {caseStudiesPage.featured.eyebrow}
                </p>
        ...
        {rest.length ? (
          <div className="mt-14 border-t border-border/60 md:mt-16">
            {rest.map((study, index) => {
              ...
              const href = localizePath(`/case-studies/${study.slug}`, locale);
              ...
              return (
                <Reveal key={study.id} delay={index * 0.05}>
                  ...
                  >
                    ...
                    <IconArrowRight
                      className={cn(
                        "hidden size-5 shrink-0 text-ink/30 transition group-hover:text-accent md:block",
                        "rtl-flip group-hover:translate-x-0.5",
                      )}
                    />
                  </Link>
                </Reveal>
